...
class Further implements Movement{
	...
	public ArrayList<ArrayList<Entity>> moveCharacter(Enemy e, Entity dest) {
		...
		if (e.checkBounds((oldX + (dX)*-1), (oldY + (dY)*-1)) && map.get(oldY + (dY)*-1).get(oldX + (dX)*-1) == null) {
			e.setMove(oldX + (dX)*-1, oldY + (dY)*-1);
		}
		//try other directions
		else if (dY == 0) {
			if (e.checkBounds(oldX, oldY+1) && map.get(oldY+1).get(oldX) == null) {
				e.setMove(oldX,oldY+1);
			}
			else if (e.checkBounds(oldX, oldY-1) && map.get(oldY - 1).get(oldX) == null) {
				e.setMove(oldX,oldY-1);
			}
			else {
		...
		//try other directions
			...
			if (e.checkBounds(oldX+1, oldY) && map.get(oldY).get(oldX+1) == null) {
				e.setMove(oldX+1,oldY);
			}
			else if (e.checkBounds(oldX-1, oldY) && map.get(oldY).get(oldX-1) == null) {
				e.setMove(oldX-1,oldY);
			}
			else {
				e.setMove(nextX, nextY);
