...
class DoorTest {
	...
	void testOpen() {
		...
		assertEquals(player.getKey(),null, "Open Door Test  - player does not have key");
		player.moveRight();
		assertEquals(player.getX(), 1, "Open Door Test");
		assertEquals(player.getY(), 0, "Open Door Test");
		assertEquals(player.getKey(),key, "Open Door Test - player has key");
		player.moveRight();
	...
	/**
	 * If a player walks over a door and does not any key, the door 
	 * remains closed.
	 */
	@Test
	void testNoKey() {
		...
		player.addObserver((playerObserver)door);
		assertEquals(player.getKey(),null, "Door Locked Test - player does not have key");
		player.moveRight();
		assertEquals(player.getX(), 0, "Door Locked Test");
	...
	void testWrongKey() {
		...
		assertEquals(player.getX(), 1, "Open Door Test");
		assertEquals(player.getY(), 0, "Open Door Test");
		assertEquals(door.getOpened(), false, "Open Door Test - door unlocked");
	}
	...
	void testStayOpen() {
		...
		player.addObserver((playerObserver)key);
		player.addObserver((playerObserver)door);
		assertEquals(player.getKey(),null, "Door Stay Open Test - player does not have key");
		player.moveRight();
		assertEquals(player.getX(), 1, "Door Stay Open Test");
		assertEquals(player.getY(), 0, "Door Stay Open Test");
		assertEquals(player.getKey(),key, "Door Stay Open Test - player has key");
		player.moveRight();
